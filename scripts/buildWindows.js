...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'RefreshView-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'RefreshView-v' + version)
  archive.pipe(output)
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/refreshview-installer' + archSuffix,
    icon: 'icons/icon256.ico',
    animation: 'icons/windows-installer.gif',
    licenseUrl: 'https://github.com/PrivacySafe/refreshview/blob/master/LICENSE.txt',
    noMsi: true
  }
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/refreshview-installer' + archSuffix + '/refreshview-' + version + '-setup.exe', './dist/app/refreshview-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
