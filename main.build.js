...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/RefreshView\/\S+\s/, '').replace(/Electron\/\S+\s/, '').replace(process.versions.chrome, process.versions.chrome.split('.').map((v, idx) => (idx === 0) ? v : '0').join('.'))
}
